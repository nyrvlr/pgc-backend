 ...
 * Endpoints do participante: acesso e respostas individuais.
 */

import { Router, type Request, type Response, type NextFunction } from 'express';
import { bearerAuth } from './auth.middleware';
import { getParticipantState } from '../services/participant.service';
import { submitJudgment, submitPunishment } from '../services/response.service';
...
// ---------------------------------------------------------------------------
// Mapeamento de SessionBootstrapError → código HTTP
// ---------------------------------------------------------------------------

function httpStatusForError(err: SessionBootstrapError): number {
...
participantRouter.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!judgment || !VALID_JUDGMENTS.includes(judgment)) {
      res.status(400).json({ error: `judgment deve ser "Just" ou "Unjust".` });
      return;
    }
    ...
    try {
      ...
      if (err instanceof SessionBootstrapError) {
        const isInvalidToken = err.message.includes('Token inválido');
        res.status(isInvalidToken ? 401 : httpStatusForError(err)).json({ error: err.message });
        return;
      }
      next(err);
    }
...
participantRouter.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!punishment || !VALID_PUNISHMENTS.includes(punishment)) {
      res.status(400).json({ error: `punishment deve ser "Punish" ou "NoPunish".` });
      return;
    }
